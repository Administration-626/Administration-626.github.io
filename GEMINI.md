...
### 3.4 任务执行协议 (Execution Protocol)
- **预确认机制**：在执行任何非查询类任务（如修改代码、新增文件、删除内容）之前，必须先明确向用户陈述**任务内容**、**实施方案**以及**任务边界**。同时，**必须提供一个清晰的代办清单 (TODO List)**，列出具体的执行步骤，并等待用户确认。
- **防止过度开发**：仅执行用户明确授权的操作，严禁擅自扩大修改范围或进行无关的重构。
